...
const el = {
    authView: requiredElement('#authView'),
    appView: requiredElement('#appView'),
    sessionLine: requiredElement('#sessionLine'),
    notice: requiredElement('#notice'),
    socketStatus: requiredElement('#socketStatus'),
    headerAvatar: requiredElement('#headerAvatar'),
    profileMenuButton: requiredElement('#profileMenuButton'),
    securityMenuButton: requiredElement('#securityMenuButton'),
    logoutButton: requiredElement('#logoutButton'),
    profileModal: requiredElement('#profileModal'),
    profileModalBackdrop: requiredElement('#profileModalBackdrop'),
    closeProfileButton: requiredElement('#closeProfileButton'),
    securityModal: requiredElement('#securityModal'),
    securityModalBackdrop: requiredElement('#securityModalBackdrop'),
    closeSecurityButton: requiredElement('#closeSecurityButton'),
    registerForm: requiredElement('#registerForm'),
    loginForm: requiredElement('#loginForm'),
    secondFactorForm: requiredElement('#secondFactorForm'),
    challengeToken: requiredElement('#challengeToken'),
    reauthForm: requiredElement('#reauthForm'),
    start2faButton: requiredElement('#start2faButton'),
    disable2faButton: requiredElement('#disable2faButton'),
    regenRecoveryButton: requiredElement('#regenRecoveryButton'),
    provisioningUri: requiredElement('#provisioningUri'),
    manualSecret: requiredElement('#manualSecret'),
    confirm2faForm: requiredElement('#confirm2faForm'),
    recoveryCodes: requiredElement('#recoveryCodes'),
    passwordForm: requiredElement('#passwordForm'),
    unlink42Button: requiredElement('#unlink42Button'),
    profileForm: requiredElement('#profileForm'),
    avatarFile: requiredElement('#avatarFile'),
    avatarPreview: requiredElement('#avatarPreview'),
    chatForm: requiredElement('#chatForm'),
    chatLog: requiredElement('#chatLog'),
    usersList: requiredElement('#usersList'),
    leaderboardList: requiredElement('#leaderboardList'),
    historyList: requiredElement('#historyList'),
    statsBox: requiredElement('#statsBox'),
    queueButton: requiredElement('#queueButton'),
    botDifficulty: requiredElement('#botDifficulty'),
    roomLabel: requiredElement('#roomLabel'),
    gameState: requiredElement('#gameState'),
    scoreBox: requiredElement('#scoreBox'),
    goalBanner: requiredElement('#goalBanner'),
    gameCanvas: requiredElement('#gameCanvas'),
    upButton: requiredElement('#upButton'),
    downButton: requiredElement('#downButton')
};
const usernameSeed = `player${Math.floor(100000 + Math.random() * 900000)}`;
formControl(el.registerForm, 'username').value = usernameSeed;
formControl(el.registerForm, 'email').value = `${usernameSeed}@example.com`;
formControl(el.loginForm, 'username').value = usernameSeed;
...
function showApp(user) {
    ...
    formControl(el.profileForm, 'displayName').value = user.displayName ?? '';
    formControl(el.profileForm, 'avatarUrl').value = user.avatarUrl ?? '';
    formControl(el.profileForm, 'bio').value = user.bio ?? '';
...
function handleLiveMessage(message) {
    ...
    if (type === 'chat:typing')
        el.gameState.textContent = 'A player is typing in chat';
    if (type === 'chat:invite')
        addSystemMessage(`${nameFor(payload.fromUserId)} invited you to play`);
    ...
    if (type === 'game:start' || type === 'game:state' || type === 'game:finished')
        updateRoom(payload);
    if (type === 'game:pause')
        el.gameState.textContent = `${nameFor(payload.userId)} disconnected`;
    if (type === 'game:resume')
        el.gameState.textContent = `${nameFor(payload.userId)} reconnected`;
...
el.registerForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        formControl(el.loginForm, 'username').value = String(body.username ?? '');
...
el.loginForm.addEventListener('submit', async (event) => {
    ...
    clearNotice();
    try {
        const data = await api('/auth/login', { method: 'POST', body: formData(el.loginForm) });
...
el.avatarFile.addEventListener('change', async () => {
    ...
    formControl(el.profileForm, 'avatarUrl').value = dataUrl;
...
el.reauthForm.addEventListener('submit', async (event) => {
    ...
    if (!body.secondFactorMethod)
        delete body.secondFactorMethod;
    if (!body.secondFactorCode)
        delete body.secondFactorCode;
    try {
...
el.passwordForm.addEventListener('submit', async (event) => {
    event.preventDefault();
    try {
        await api('/auth/password/change', { method: 'POST', body: formData(el.passwordForm) });
...
el.chatForm.addEventListener('submit', (event) => {
    event.preventDefault();
    const body = formControl(el.chatForm, 'body').value.trim();
    if (!body)
        return;
    sendLive('chat:send', { body });
    el.chatForm.reset();
});
formControl(el.chatForm, 'body').addEventListener('input', () => sendLive('chat:typing'));
...
el.usersList.addEventListener('click', async (event) => {
    ...
    if (!userId || userId === state.me?.id)
        return;
    try {
        if (button.dataset.action === 'friend')
            await api(`/users/${userId}/friends`, { method: 'POST' });
        if (button.dataset.action === 'invite')
            sendLive('chat:invite', { toUserId: userId });
...
const table = new THREE.Mesh(new THREE.BoxGeometry(12.5, 7.5, 0.16), new THREE.MeshStandardMaterial({ color: 0x22313f, roughness: 0.55 }));
table.position.z = -0.12;
scene.add(table);
const centerLine = new THREE.Mesh(new THREE.BoxGeometry(0.08, 7.1, 0.04), new THREE.MeshBasicMaterial({ color: 0xd7f9f1 }));
